...
function Decom() --Start Decom. Activates alarm, message and functions. Defines Suffering() which is the actual killing
    ...
    function cough() --make each plr in lcz cough every 4 secs
        plr_loop(function(plr) 
            if getplayertype(plr) == 0 or getplayerzone(plr) ~= 1 or ifSCP(getplayertype(plr)) then return end
...
function OnRoundStarted()
    ...
    function decom_annouc(mins) --Decom annoucement. Will call decom timer when 10 mins to decom start        
        --Minute = 60000
        --5 mins = 300000
        timers = {} --Wipe list. (Declared here for commands)
        mins = tonumber(mins)
        servermessage(string.format("[FACILITY] LCZ Decomtamination Process will begin in T-Minus %d Minutes",mins)) --Alert Facility of incoming doom
        alarm()
        if mins > 10 then
            ...
            local secs
            if mins - 5 < 10 then
                secs = (mins - 10)*60000--Remember to change to 60000
                mins = 15
            else secs = 300 end
...
function OnPlayerConsole(plr,msg)
    ...
    local select = {
        ...
        ["enddecom"] = function() -- Use console to cancel decom
            if not timers[3] then sendmessage(plr, "[DECOM] Decomtamination Procedure is not currently active")
            else
    ...
    if string.find(msg, "decomtimer") then
        ...
        if type(tonumber(msg)) == "nil" then sendmessage(plr,"[DECOM] Error, Decomtamination Timer could not be set")
        else createtimer("decom_annouc",2000,0,tonumber(msg)) end
